...
| `seerr-integration.js` | Seerr routes, ratings resolution, card/detail rendering, sorting, filtering, bulk selection, Plex Watchlist button on detail pages |
| `seerr-api-observer.js` | Page-world observer forwarding the title lists Seerr fetches for itself |
| `background.js` | Seerr REST requests, Plex Watchlist writes via plex.tv Discover, RT search and scorecard parsing, settings migration, toolbar badge |
...
Messages use an `action` plus action-specific fields. Request/status/watchlist/RT actions use `data`; search and debug helpers also have legacy top-level arguments. Replies are `{ success: true, data }` or `{ success: false, error }`. Plex actions are `plexTestConnection` and `plexAddToWatchlist`: the worker holds the Plex token device-local and content scripts only learn `plexConfigured` via `getConfigState`, mirroring the Seerr key contract. Plex resolution prefers an exact TMDB GUID match from Discover metadata and refuses ambiguous title-only matches; the add is a single PUT, never retried.
...
Active settings are `seerrUrl` and `overlayFeatures` in `storage.sync`, and `seerrApiKey`, `plexToken` and `debugLogging` in `storage.local`. `debugLogging` is a Settings checkbox and gates all worker tracing; errors are never gated. The keys are device-local because sync replicates through the browser account; the cost is re-entering them per device. Features default on unless explicitly false. Changes apply after Save Settings and trigger overlay cleanup/reinjection. The popup asks the worker for separate Seerr connectivity, host permission, API-key acceptance, and Plex diagnostics. Diagnostic responses omit credentials and remote error bodies; repair links focus the matching Settings control. The worker records up to 50 confirmed requests and watchlist additions in `recentActionsV1`, using an allowlist of action type, title, media type, and timestamp. Writes and clears serialize; history failures never change the outcome of an already confirmed server action. History reads/clears through messages are limited to extension pages. Local storage is restricted to trusted extension contexts before initialization where the browser supports `setAccessLevel`; Firefox 147.0.3 lacks that API.
...
IMDb comes solely from `/ratingscombined`, and some servers cannot reach the sources behind Seerr's ratings endpoints, so those 404 for every title. After a sustained run of 404s the overlay stops asking for the rest of the session, since each ask costs a request and a console error for data that will not arrive. Failures are counted per endpoint, because the two fail independently: Seerr answers `/ratingscombined` with 200 when it holds either source, so a server with IMDb but no Rotten Tomatoes succeeds there and 404s on `/ratings` for every card, and a shared counter would be reset by those successes. The implication runs one way — a combined 404 means neither source exists, so `/ratings` is dropped with it, while a `/ratings` 404 says nothing about IMDb. Any success resets that endpoint, so an intermittent server is not abandoned, and **Refresh scores** clears both. `diagnose()` reports the state. This step runs once per card, so walking every endpoint regardless would multiply load on the server.
...
The overlay coalesces pending lookups and sends debounced persistence messages to the worker. The worker validates the extension sender, top frame, exact configured origin and base path; returns only the bounded ratings/index schema; and serializes writes and clears in `storage.local`. Content scripts never directly read or write local storage. Safe runtime notifications propagate clears and configuration changes without credentials. Scored entries render immediately and revalidate after 24 hours (one hour for partial bundles); unrated entries retry after six hours. Failed refreshes retain known scores and retry after two minutes. Eviction is least-recently-used, and Settings clears the bounded caches. The title index behind the cards persists in the same write (bounded separately), so a cold load identifies cards before any network answers; the first grid pass waits for that read, and unloading flushes pending writes instead of losing them. Each records when it was resolved, which the grid reports so a stale set is visible; entries written before that field existed load as a bare bundle with no recorded age. **Refresh scores** forgets the titles currently on screen and resolves them again, passing `refresh` through to the worker so its own 24-hour Rotten Tomatoes entry is discarded too — without that the score most likely to have moved is the one that would not. The overlay and Rotten Tomatoes caches are bounded separately (`overlayCacheMaxEntries`, `rtCacheMaxEntries`), since their entries differ in size and expiry and each cap also decides how large that cache's rewritten blob gets — at 5000 apiece they come to roughly 2 MB together, against a 10 MB default quota. A stored blob records which server it came from and is discarded on mismatch, and a bundle carrying no scores at all is never stored, since without an expiry that would mean never looking again. Because clearing is a key removal, the overlay treats only a removal as a clear and ignores its own writes. The worker separately coalesces identical RT title/type/year requests across callers. Positive RT results cache for 24 hours; low-confidence/no-match results for one hour. Rejections clear pending entries so later attempts can retry. Caches are bounded. The worker cache is written through to `storage.local` under a single key with a short debounce, because MV3 evicts the worker after seconds of idle and an in-memory map alone could never reach the 24-hour TTL. A corrupt or expired persisted entry degrades to a cold lookup. Network requests have a ten-second deadline.

A Seerr detail heading carries its year, as in “Moana 2 (2024)”. Left whole that title only substring-matches Rotten Tomatoes’ “Moana 2”, and the year it carries goes unused, so a correct match came back uncertain. Resolution splits a parenthesised trailing year off any title it is given and uses it when no other year is known; a number that is part of the title, as in “Blade Runner 2049”, is untouched.

Rotten Tomatoes lists most films under their English title, so a Seerr showing localised titles gives a name it does not carry. Both the localised and the original title are tried, in that order, deduplicated by the same normalisation used for matching. Candidates are scored against every title the film is known by rather than against the query, because searching a localised name returns the film under its English one and judging that by the query would reject the right answer.

Titles are compared after normalising: accents are decomposed so an accented spelling matches an unaccented one, the Latin letters that carry no combining mark are mapped by hand, and letters of any script are kept. Restricting this to `a-z` turned an accent into a space, splitting the word it sat in, and reduced a title in a non-Latin script to an empty string that could never match.

RT search parsing and title matching are heuristic, not an official guaranteed RT API. An exact title in the right year scores 1 and is shown plainly; anything less is marked approximate with a `~`, which is only meaningful because full confidence is reachable. A title with no year to corroborate it can never be certain, and where two candidates match such a title equally well the lookup returns nothing rather than take Rotten Tomatoes' own ordering as the answer — several films can share a title exactly. Markup changes can still yield no score or a wrong match.
...
Tests include fast-check properties, worker execution in a VM, jsdom execution of production UI code and of the shared integration layer, and builds in temporary directories. Mocked API/DOM tests do not prove production site compatibility or authenticated server behavior. Firefox 147.0.3 passes the real-extension local-fixture smoke test; authenticated live-site and seven-site coverage remain separate gates (see README).
