...
test('a server that never has combined ratings stops being asked', async () => {
  ...
  const attempts = Config.combinedRatingsFailureLimit + 25;

  for (let i = 0; i < attempts; i++) await overlay.fetchSeerrSessionRatings(1000 + i, 'movie', null);

  assert.ok(combinedCalls() <= Config.combinedRatingsFailureLimit,
    `expected it to give up after ${Config.combinedRatingsFailureLimit}, made ${combinedCalls()} calls`);
...
test('other endpoints keep working once it gives up', async () => {
  ...
  for (let i = 0; i < Config.combinedRatingsFailureLimit + 5; i++) {
...
test('a single success keeps the endpoint in use', async () => {
  ...
  const attempts = Config.combinedRatingsFailureLimit + 10;
  for (let i = 0; i < attempts; i++) await overlay.fetchSeerrSessionRatings(1000 + i, 'movie', null);
...
test('an intermittent server is not given up on', async () => {
  ...
  const attempts = Config.combinedRatingsFailureLimit * 3;
  for (let i = 0; i < attempts; i++) await overlay.fetchSeerrSessionRatings(1000 + i, 'movie', null);
...
test('diagnose reports that it gave up, rather than hiding it', async () => {
  const { overlay } = overlayWithSeerr();
  for (let i = 0; i < Config.combinedRatingsFailureLimit + 2; i++) {
    await overlay.fetchSeerrSessionRatings(1000 + i, 'movie', null);
  }
  const report = overlay.context.window.seerr_debug.ratings.diagnose();
  assert.equal(report.combinedRatings.givenUp, true);
  assert.ok(report.combinedRatings.consecutiveFailures >= Config.combinedRatingsFailureLimit);
});
